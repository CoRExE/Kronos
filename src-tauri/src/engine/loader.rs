...
pub fn load_data(conn: &Connection) -> Result<EngineInput, String> {
    ...
    // 4. Allocations
    let mut stmt = conn.prepare("SELECT id, group_id, subject_id, teacher_id, count, required_room_type FROM allocations").map_err(|e| e.to_string())?;
    let allocs_iter = stmt.query_map([], |row| {
        Ok((
            ...
            row.get::<_, i32>(4)?,
            row.get::<_, String>(5)? // required_room_type
        ))
    ...
    // 5. Config globale (Version corrigée et simplifiée)
    ...
    Ok(EngineInput {
        time_slots, slot_day_map, rooms, allocations: allocations_to_place,
